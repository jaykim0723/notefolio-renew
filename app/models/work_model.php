...
class work_model extends CI_Model {
    ...
    function get_list_prep($params){
        ...
        if($params->from!='all'){
            $this->db->having("(regdate >= ".$this->db->escape($from).")", NULL, FALSE); // 모든 기준이 regdate로 하기 때문에
            // $this->db->having("(regdate >= ".$this->db->escape($from)." or works.moddate >= ".$this->db->escape($from).")", NULL, FALSE);
        }
