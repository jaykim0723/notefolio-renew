...
class Auth extends CI_Controller
{
    ...
    function _login_after($is_success, $go_to, $data)
    {
        ...
        if ($is_success) {
            ...
            $data['auth_error'] = $this->tank_auth->get_error_message();
...
//                    'term'          => $ci->form_validation->set_value('term'),
            ...
            $this->form_validation
                ...
                ->set_rules('username', '개인 URL', 'trim|required|alpha_dash|check_username_available|xss_clean|is_unique[users.username]|min_length['.$this->config->item('username_min_length','tank_auth').']|max_length['.$this->config->item('username_max_length','tank_auth').']')
                ->set_rules('realname', '이름', 'trim|required|max_length[45]')
...
//                ->set_rules('term', '약관 동의', 'trim|required')
	...
	function check_username_available ($username='') {
        ...
        if($return){
            ...
            ->set_rules('username', '개인 URL', 'trim|required|alpha_dash|xss_clean|min_length['.$this->config->item('username_min_length','tank_auth').']|max_length['.$this->config->item('username_max_length','tank_auth').']')
            ->run();
	...
	function reset_password()
	{
		...
		$this->form_validation->set_rules('new_password', '새로운 비밀번호', 'trim|required|xss_clean|min_length['.$this->config->item('password_min_length', 'tank_auth').']|max_length['.$this->config->item('password_max_length', 'tank_auth').']|alpha_dash');
		$this->form_validation->set_rules('confirm_new_password', '새로운 비밀번호 확인', 'trim|required|xss_clean|matches[new_password]');
		...
		if ($this->form_validation->run()) {								// validation ok
			if (!is_null($data = $this->tank_auth->reset_password(
				...
				$this->_show_message($this->lang->line('auth_message_new_password_activated').'<br />'.anchor('/auth/login/', 'Login'));
