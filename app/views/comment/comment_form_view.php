<form action="" class="comment-block create" data-mode="create" data-parent_id="" data-comment_id="">
	...
	<a class="comment-profile-area" href="<?=(USER_ID>0)?site_url($this->session->userdata('username')):'javascript:return;'?>" <?php if(USER_ID>0){?>target="_blank"<?php }?>>
		<?php if (USER_ID==0): ?>
			<img class="icon-round" src="/img/default_profile_face.png"/>
		<?php else: ?>
			<img class="icon-round" src="<?php echo '/data/profiles/'.$this->session->userdata('username').'_face.jpg?_='.substr($this->nf->get('user')->modified,-2) ?>" alt="" onerror="this.src='/img/default_profile_face.png'"/>
		<?php endif ?>
