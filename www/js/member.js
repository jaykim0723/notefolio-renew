	...
	save : function(form, returnType){
		...
		$('#content-block-list > li').each(function(index){
    		...
    		switch(type){
    			...
    			case 'text': 
    				o.c = encodeURIComponent(br2nl($(this).find('textarea').val())).replace(/%5C/gi, '%22%5C');
    			break;
		    ...
		    message: function(){
				var $message = $(
					'<div id="dialog-change-realname">'+
						...
						'<input type="text" class="form-control" value="'+$.trim($('#profile-info > h2').text())+'"/>'+
					'</div>'
					);
				return $message;
		    },
			...
			$.get(site.url+'/profile/statistics_datatable', {
				...
				$('#statistics-table-area').html(html).children('table').dataTable({
					"oLanguage": {
						...
						"sInfo": "전체 _TOTAL_작품 중 _START_-_END_번",
						"sInfoEmpty": "전체 0작품 중 0-0번",
						"sInfoFiltered": "(전체 _MAX_작품 중에서 필터링 하였습니다)",
						"sSearch": "검색:",
